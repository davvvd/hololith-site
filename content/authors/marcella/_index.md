---
# Display name
title: Marcella Salvatore # <--------------


# Full name (for SEO)
first_name: Marcella # <--------------
last_name: Salvatore # <--------------


# Username (this should match the folder name)
authors:
  - marcella # <--------------


# Role/position
role: Assistant Professor # <--------------

                    
# Organizations/Affiliations
organizations:
  - name: University of Naples Federico II - Physics Department # <--------------
    url: 'https://www.fisica.unina.it/' # <-------------- OPTIONAL
  - name: ''
    url: ''

# Short bio (displayed in user profile preview)
bio: Marcella Salvatore holds a Ph.D. in Industrial Product and Processes Engineering (University of Naples, 2017) and an M.Sc. in Materials Engineering (2013). Her work focuses on azomaterials surface engineering and tailoring surface functionality for wettability control and optical devices. She has authored 45 peer-reviewed publications (h-index 15) and is active in international collaborations and conference presentations.


interests: # <--------------
  - Material science
  - Laser processing
  - Azobenzene containing materials


education: # <-------------- EDIT BELOW (YOU CAN ADD MORE ENTRIES)
  courses:
    - course: PhD in Industrial Product and Processes Engineering
      institution: University of Naples "Federico II"
      year: 2017
    - course: Master degree in Materials Engineering
      institution: University of Naples "Federico II"
      year: 2013
    - course: Bachelor degree in Science and Materials Engineering
      institution: University of Naples "Federico II"
      year: 2011


# Social/Academic Networking
# For available icons, see: https://docs.hugoblox.com/getting-started/page-builder/#icons
#   For an email link, use "fas" icon pack, "envelope" icon, and a link in the
#   form "mailto:your-email@example.com" or "#contact" for contact widget.

social:
  - icon: envelope
    icon_pack: fas
    link: 'mailto:marcella.salvatore@unina.it' # <--------------
  
  - icon: google-scholar
    icon_pack: ai
    link: 'https://scholar.google.it/citations?hl=it&user=t7Z3NisAAAAJ'

 


# Organizational groups that you belong to 
user_groups:
  - Researchers  # <-------------- one or more in - Principal Investigators
                 #                                - Researchers
                 #                                - PhD Students
                 #                                - Visitors
---

Marcella Salvatore received her M.Sc. in Materials Engineering in 2013 and earned a Ph.D. in Industrial Product and Processes Engineering from the University of Naples in 2017. Her research centers on azomaterials surface engineering, with a particular emphasis on tuning surface functionality through innovative surface designs. She investigates mass-transport phenomena in azomaterials and develops functional surfaces aimed at applications such as wettability control, optical components, and photonics-related devices.

With a strong materials-science background, she has contributed to multiple cutting-edge research projects and regularly disseminates her results at international conferences in materials science, optics, and photonics. Marcella has published 45 peer-reviewed articles in leading scientific journals and currently has an h-index of 15, reflecting her sustained impact in the field.
